...
def maincite(setinp):
    ...
    # setinp list to take input from Test Script
    if setinp is None:
        inp = input("Enter URL: ")
    else:
        inp = setinp[0]
    ...
    choice = True
    while choice:
        result_set = []     # For sending results to test script
        try:
            if setinp is None:
                ch = input(usr_msg)
            else:
        ...
        if ch == '1':
            ...
            if X in ref:
                ...
                for item in ref[X]:
                    print(X + ": " + item)
                    result_set.append(X + ": " + item)
            else:
                print("Sorry ! No citations found.")
                result_set.append("Sorry ! No citations found.")
        ...
        # Get citations of a line
            ...
            result_set.append("Invalid Choice ! Re-enter: ")

        # To avoid Infinite loops from test suite
        if setinp is not None:
            choice = False
